...
- A new standalone frontend: `index.html` — a single-file, client-side app that you can open directly in your browser (double-click the file) without running a server.
...
- Splits recipients into batches (configurable batch size, default 100).
...
3. Paste or upload your recipients (CSV single-column). Enter subject and body, set batch size, and click "Generate links".
4. Click a link to open the Gmail composer with fields pre-filled, then click Send.

Notes and limitations
- Sender address cannot be forced by the Gmail compose URL. The sender field is informational — the user must select the correct From address in Gmail if they have multiple accounts configured.
- URL length limits and Gmail recipient limits may prevent very large batches from working reliably. The app enforces a 50,000 recipient maximum but you should use conservative batch sizes (e.g., 100).
